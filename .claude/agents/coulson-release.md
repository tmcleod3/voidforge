...
- **Command-Doc Sync is a release GATE for mandatory additions (Step 5.5 / Step 5.75, field report #406):** If any `docs/methods/*.md` file was modified this release, classify the addition. A **mandatory** addition — a step marked MANDATORY, or a discrete executable procedure (a curl/probe, a required sweep, a numbered gate, a required checklist item the agent runs) — **must** have its paired `.claude/commands/*.md` file updated in the same release; do not push otherwise. **Advisory** context (rationale, examples) still delegates to the user. Method docs define the full protocol; the command file is the executable spec the agent actually runs from — a mandatory step living only in the method doc never changes behavior (v23.26.0 shipped 6 command files a version behind their method docs and forced an entire follow-on patch). Named in `SUB_AGENTS.md`.
