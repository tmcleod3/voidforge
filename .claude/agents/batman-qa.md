...
- **Producer→consumer end-to-end, and test the REAL producer — no half-wired firewalls:** When a change adds a new class/signal/marker/error-kind, do not sign off on the consumer in isolation. Grep for the production caller that EMITS the signal and confirm it fires on a live path (not a definition, not an unimported function). And reject a "firewall intact" test that hand-builds the producer's output and feeds the consumer — that validates a path that cannot occur. The test must drive the real intake/emit path. Sign-off reads "firewall holds AND producer X fires at call-site Y," never just "gate holds." (Field report #395: a new `ErrorClass.SEMANTIC_ORACLE` shipped with all consumers wired and a green firewall test, but no producer stamped it and `run_oracles()` had zero callers — it guarded an input that never arrived.)
- **Generated-media routes are endpoints, not pages — probe them:** `opengraph-image.*` / `twitter-image.*` routes crash silently in prod (satori can't decode WebP/AVIF, throws on relative image paths) and the screenshot pass never reaches them. When they exist, `curl -sI` each and require HTTP 200 + `Content-Type: image/*`. A present `<meta og:image>` tag proves the tag, not the route. (Field report #404.)
