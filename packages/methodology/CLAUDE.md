...
3. **Normalize roster names before dispatch** (field report #345, DEAL-001). The Silver Surfer returns agent names from a Haiku pre-scan, which can drift from the actual filenames in `.claude/agents/` (extra `silver-surfer-` prefix, a stray `.md` suffix, a hyphen/underscore mismatch). Before you launch, validate each name against `ls .claude/agents/`: if it matches a file (with or without the `.md` extension), keep it; if not, attempt exactly one correction — strip a known prefix/suffix or normalize separators — and re-check. If it still doesn't resolve, DROP that single name and proceed with the rest of the roster. Never block the whole dispatch over one unresolved name; log the dropped name so the Herald roster can be corrected upstream. The gate's job is to enforce *that* a roster ran, not to fail the run over a typo in *one* name. **Workflow-dispatch caveat (field report #398):** the Agent tool's `subagent_type` and the Dynamic Workflow tool's `agentType` both resolve by the agent's registry **display name** (`Spock`, `Gaal Dornick`, `The Mule`), NOT the `.claude/agents/` basename (`spock-schema`, `gaal-prompt-arch`, `the-mule`). Basename validation confirms the file exists; it does NOT produce a launchable `agentType`. When dispatching via a workflow script, convert each validated basename to its display name (read the agent file's `name:` field) BEFORE writing it into the script — a basename-style `agentType` silently returns `null` and the agent never runs (WORKFLOWS.md Gotcha 6).
...
- `git-sha-resolve.sh` — robust git-SHA resolution (loose ref → `packed-refs` → `git rev-parse` fallback) so a `/healthz` `git_sha` doesn't degrade to the raw `ref:` string after `git gc` packs the branch ref (field report #397)
