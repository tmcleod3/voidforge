...
6.1. **File the issue before citing its number in any committed artifact (field report #393).** GitHub issue and PR numbers share one unpredictable sequence — you cannot predict the number a `/debrief --submit` will receive (something may take the next number between your write and your file). Never bake a not-yet-created issue number into docs, `.gitignore` comments, code comments, or CHANGELOG text: either file the issue first and cite the real number, or use a stable placeholder (`#TBD`) and substitute it before commit. (v23.24.0 shipped four references to a predicted "#391" that came back as #392 — four post-hoc corrections.)
