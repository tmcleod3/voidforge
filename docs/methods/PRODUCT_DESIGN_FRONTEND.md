...
13. **Read the positioning constraint BEFORE writing public copy, SEO, or marketing phrases (field report #404).** The engine's *capability* scope is not the product's *marketing* scope. Before writing any public-facing copy, page title/description, JSON-LD, og text, loader phrases, or SEO content, read the PRD's positioning field and confirm which games/categories/brands are in scope for **public messaging**. A multi-game engine may be marketed as single-game by operator choice — do not assume "the engine supports X, Y, Z" means "market X, Y, Z." (Snug Cards: an SEO batch optimized copy toward multi-game positioning to match the multi-game engine; the operator's actual constraint was "public positioning = Pokémon only," forcing a same-day full copy-scrub reversal. The engine stayed multi-game; the marketing didn't.)
...
7. **Interaction-path completeness for animations & triggered states (field report #402/#404).** "Screenshot every page" and "every surface that consumes the changed prop" both address *pages* and *props* — neither covers a behavior that fires from **more than one trigger path**. A celebration/animation/transition verified via ONE trigger is NOT covered. For every multi-trigger behavior, **enumerate every trigger path, exercise each, and screenshot/record the result of each**; flag any path not covered by E2E as incomplete coverage. A feature that is correct code but invisible in one of its trigger paths has NOT shipped in that path. (Snug Cards: a gold-sweep celebration was verified only via the list-view pip-toggle; its primary path — sheet-own → close → sweep on the pocket grid — played the animation behind the blurred modal overlay, self-clearing unseen. The code was correct; one of its two trigger paths was never exercised, so the feature was invisible where users actually met it.)

**Generated-media route probe — MANDATORY when og/twitter image routes exist (field report #404).** `opengraph-image.*` / `twitter-image.*` (and any `route.ts` that returns an image) are **backend image-generation endpoints, not pages** — unreachable by browser click, so the screenshot pass never exercises them, and checking that the `<meta property="og:image">` tag *exists* only proves the tag, not that the route renders. They crash silently and surface only when a social scraper (Twitter/Slack/Discord) fetches them in production. Probe each over HTTP: `curl -sI <base>/<route>/opengraph-image.<ext>` MUST return **HTTP 200** AND **`Content-Type: image/*`**. A meta tag pointing at a route that 500s is a CRITICAL finding regardless of the tag's presence, and a "proven pattern" must be verified in THIS session — never cited by reputation. **Satori pitfalls (encode as named traps, like the Tailwind gotcha):** `next/og` / satori (1) throws on **relative** image paths in `ImageResponse` (requires absolute URLs), and (2) cannot decode **WebP/AVIF** — fetch the bytes in-route, magic-byte-sniff, and hand satori a data URI of a decodable format only. (At snug.cards both binder and profile og routes had crashed in prod since they shipped — image cache was ~99% WebP — yet passed two full review rounds that checked only the meta tag.)
