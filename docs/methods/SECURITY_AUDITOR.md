...
**Adjacent plaintext credential-store sweep (off-repo surfaces) (field report #394):** Per-mission and per-diff reviews see only their own changeset — they are structurally blind to plaintext credential stores that live *outside the repo* on the host. A cross-cutting security pass (and every credential-migration campaign) MUST explicitly sweep the adjacent stores that no diff ever touches: `~/.git-credentials`, `~/.netrc`, `~/.aws/credentials` + `~/.aws/config`, `~/.docker/config.json`, `~/.npmrc` / `~/.pypirc` (auth tokens), `~/.ssh/` key permissions, `~/.config/gh/hosts.yml`, `~/.kube/config`, and **DB-dump / backup file permissions** (`/…/backups/*.sql*`, `*.dump` — confirm they are not group/world-readable by a different service user). Flag any plaintext long-lived credential or over-permissive dump as HIGH+. (Field report #394: a whole-system Victory Gauntlet — not any per-mission review — found `~/.git-credentials` holding unused plaintext GitHub PATs and prod DB backups group-readable by the staging user; neither was "in" any mission's diff, so only a cross-domain sweep could see them.) See `docs/patterns/credential-store-migration.md` for the deletion-side gate.
