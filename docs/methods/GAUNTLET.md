...
**Verify budget applies in infinity mode too.** The Step 4.5 verify scope (Critical/High → 3-lens; Medium → batched; Low/Warn → advisory) and `VERIFY_AGENT_BUDGET` apply equally in infinity mode — do NOT 3-lens-verify Medium/Low findings, or a large codebase will breach the ~1,000-agent cap (field report #405).
