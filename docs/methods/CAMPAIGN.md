...
5.2. **Re-verify each review-era finding's premise against current HEAD before a mission applies it (field report #402/#404).** Findings authored at review time (a `/ux` mega-review, an earlier gauntlet) carry an *implicit timestamp*. By the time a later mission addresses one, code may have shipped, data may have been corrected, or another mission may have filled the gap — so the finding's premise can be stale even though the finding was accurate when written. Before building a fix, confirm the stated problem still exists at HEAD (grep/read the cited artifact). If the premise no longer holds, mark the finding **stale**, skip it, and log the premise-drift in `campaign-state.md`. This extends rule 7a (#360 premise verification) to the specific case of review findings aging across a multi-mission campaign. (Snug Cards: F5/F9/B4 were premise corrections — aria-pressed and aria-labelledby already present, tile names already on the display font — NOT regressions; caught pre-build in minutes, but only because the orchestrator applied #360 voluntarily. This rule makes it non-optional.)
5.3. **Update the docs of record per-mission during a multi-phase production change (field report #394).** Set ADR **Status** and ROADMAP mission checkmarks at *each mission commit*, not at end-of-campaign. An ADR/ROADMAP that still reads "NOT IMPLEMENTED" while the system is live post-cutover misleads the next operator who reads it as the operational reference. Docs of record must track live state as phases ship.
...
### Hardware-in-loop features (field report #404)

When a feature's correctness depends on a physical device the build environment cannot exercise — a gyroscope/accelerometer (tilt, orientation), camera, microphone, GPS, NFC, haptics, a Bluetooth peripheral — mark it a **named category** at plan time and gate its mission on an **"operator has tested on real hardware"** checkpoint before it ships. Hardware testing is a **pre-merge gate, not a post-ship discovery tool**. Two forces make this necessary: (1) the naive implementation uses **raw sensor data** (natural for a sensor feature) instead of a **behavior-first spec** ("the card leans toward the ground when I tilt the phone" — a gravity-derived vector — not "map beta/gamma to rotation"); (2) the operator often cannot articulate the intended behavior until they hold v1 in their hand. Capture the behavior as a discrete spec choice in the PRD (e.g. *tilt* vs *rotation* are different intents), and expect ≥1 hardware round-trip. (Snug Cards HoloTilt: v1 raw beta/gamma was orientation-naive; v2 derived gravity but added a plumb rotateZ so the card *rotated* instead of *tilted*; v3 removed rotateZ — two operator hardware tests to converge on "tilt, not rotation.")

### Live-arming — a distinct phase after the build (field report #399)

**"Built + fixture-tested green" ≠ "live-verified."** For any campaign that builds an autonomous or agentic capability — a QA-detector suite, a monitoring oracle, a crawl/scanner, a firewall/gate that keys off runtime signals — add an explicit **live-arming phase** after the build missions, before Victory. Fixtures mock the page/DB/config, so they *systematically* miss a class of failures that only appear against a real running app: async render timing (a detail rendered by a 2nd fetch after `networkidle` reads an empty frame), list cap/sort, env/config preconditions, and — most treacherous — **the harness mutating its own fixtures**. Requirements of the phase:

1. **Run each detector/oracle against a real seeded staging app and require a planted-defect RED on the LIVE surface** — not just a fixture RED. A detector that never went red on a live planted bug is unproven, regardless of its unit tests.
2. **Verify a fix in the SAME context it failed.** An agentic QA crawl that runs CRUD flows is *hostile* to any oracle asserting that data: an isolated/pristine run passes every time while the full live crawl fails. Assert a **stable, deterministic surface**, and run **pristine-seed oracles FIRST, before the mutating flows** — an oracle that asserts seeded data cannot run after agentic CRUD flows in the same run (the crawl may delete its own seeded entities before the oracle reads them).
3. **Adversarially audit your own completion claims against the committed code.** Re-derive each "done"/"the firewall now fires live" claim from the code and the live cadence, not from memory — the producer wired into the library/CLI but never passed the DSN at the live cadence fires **zero** times while the orchestrator reports it armed. A self-audit phase that re-checks each claim against the code catches orchestrator overclaim before it reaches the operator. (See also QA_ENGINEER.md "Producer→Consumer End-to-End Trace.")
