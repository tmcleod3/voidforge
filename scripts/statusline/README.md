...
- **`statusLine` is a single-winner setting across the whole hierarchy.** Claude Code renders exactly one status line; the highest-precedence file that defines it wins — **Local (`.claude/settings.local.json`) > Project (`.claude/settings.json`) > User (`~/.claude/settings.json`)**. So a `statusLine` in a higher scope shadows a lower one (e.g. a Local one shadows the Project meter), with no error — the meter just doesn't appear. Both **`/contextmeter`** and **`npx voidforge-build update`** handle this for you: when they detect a competitor in Project/User scope, they **escalate the meter to Local scope** (`.claude/settings.local.json`, which outranks Project and User) so it wins **without modifying your global `~/.claude/settings.json`** (field reports #390, #392). (If the competitor is itself a Local statusLine, the non-interactive `update` won't clobber it — it warns and defers to `/contextmeter`.) `/contextmeter --status` scans the full hierarchy and reports "installed but shadowed by `<file>`" if a competitor still occupies the slot; `--force-global` is the explicit opt-in to set the meter in your global file (with a backup). Note: `.claude/settings.local.json` is per-machine and should be gitignored.
