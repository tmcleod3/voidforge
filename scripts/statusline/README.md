...
- **`statusLine` is a single-winner setting across the whole hierarchy.** Claude Code renders exactly one status line, so a `statusLine` in `~/.claude/settings.json` (e.g. one the native `/statusline` command wrote — a common global setup) or in `.claude/settings.local.json` will **shadow this project meter even when the project install is correct**. The meter just won't appear, with no error. If yours doesn't show, check `~/.claude/settings.json` for a competing `statusLine` and remove or merge it; `/contextmeter --status` scans the full hierarchy and reports "installed but shadowed by `<file>`" when this is the case (field report #390).
